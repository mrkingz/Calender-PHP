...
class VerificationCode
{
   ...
   /**
     ...
     * @return bool
     */
    protected function expiredCode($createdAt)
    {
    ...
    /**
     * Update user and fire the Verified code event.
     *
     * @return void
     */
    protected function verify($user)
    {
        $user->active = User::VERIFICATION; 

        $user->save();
       
        event(new VerifiedCode($user));
    ...
    {
        $response = $this->checkCode($code);

        // If response is null, then code is valid
        // Verify user and delete the verification record
        if (is_null($response)) {

            $this->verify(Auth::user());
